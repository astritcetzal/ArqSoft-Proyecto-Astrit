    ...
    ""Razon"": ""Razón corta""
        ...
        public async Task<List<Book>> ExtraerLibrosDeTextoAsync(string textoUsuario)
        {
            ...
            var requestBody = new
            {
                ...
                messages = new[]
                {
            new { role = "system", content = "Extrae libros de textos en español. Responde ÚNICAMENTE un arreglo JSON plano, sin markdown." },
            new { role = "user", content = $@"Extrae los libros del texto y clasifícalos en: 'Terminado', 'Leyendo' o 'Pendiente'.
        Texto: ""{textoUsuario}""
        Formato:
        [
          {{ ""Titulo"": ""..."", ""Autor"": ""..."", ""Estado"": ""Pendiente"" }}
        ]" }
                }
                    };
            ...
            try
            {
                ...
                var jsonResponse = await response.Content.ReadAsStringAsync();
                using var doc = JsonDocument.Parse(jsonResponse);
                string texto = doc.RootElement.GetProperty("choices")[0].GetProperty("message").GetProperty("content").GetString() ?? "[]";
